...
vi.mock("better-auth/next-js", () => ({
  toNextJsHandler: vi.fn(() => ({
    GET: vi
      .fn()
      .mockResolvedValue(new Response("GET handler", { status: 200 })),
    POST: vi
      .fn()
      .mockResolvedValue(new Response("POST handler", { status: 200 })),
  })),
}));
...
describe("Auth Route Handler - Turnstile Validation", () => {
  ...
  describe("Development Environment", () => {
    it("POST to /api/auth/sign-in validates Turnstile token", async () => {
      const { verifyTurnstileToken, getClientIp } = await import(
        "@/lib/turnstile"
      );
